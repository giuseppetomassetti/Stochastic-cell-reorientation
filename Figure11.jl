# ============================================================================
# Code to generate Figure 11: mu_comparison_second_spreading.png and kappa_comparison_second_spreading.png
# ============================================================================
"""
This code generates Figure 11, which consists of two comparison plots demonstrating the 
"two-stage reorientation" phenomenon:
  - Plot (a): mu_comparison_second_spreading.png — mean orientation angle μ/π vs t/t_max
  - Plot (b): kappa_comparison_second_spreading.png — concentration parameter κ vs t/t_max
...
2. Parameters (can be modified):
   - Line 93: T = 10.0 — Time span for the simulation
   - Line 94: ε = 1.0 — Strain amplitude
   - Line 98: τ = 0.1 — Pseudo-temperature
   - Line 115: k2 = 1.2 — Stiffness parameter (overrides computed value)
   - Line 121: mu0 = 1e-2 * π — Initial mean orientation angle (very close to 0, near energy maximum)
   - Line 122: κ_values = [1, 10.0, 50.0] — Initial concentration parameter values to compare
...
3. Run the code:
   ...
   Or in Julia REPL:
   ```julia
   julia> include("Figure11.jl")
   ```

4. Output files:
   - mu_comparison_second_spreading.png (saved at line 359)
   - kappa_comparison_second_spreading.png (saved at line 382)

WHAT THE CODE DOES:
==================
- Uses von Mises initial distribution with μ₀ ≈ 0 (very close to energy maximum) and different κ₀ values
...
- Demonstrates the "two-stage reorientation" phenomenon where cells start near an energy maximum

KEY FEATURES:
============
- Initial condition: μ₀ = 1e-2 * π ≈ 0.0314 (very close to 0, near energy maximum)
- Multiple κ₀ values: [1, 10.0, 50.0] (higher initial concentrations)
- Two-stage reorientation: The concentration parameter κ first decreases (spreading) 
  before increasing (concentrating) at the energy minimum
- This phenomenon is most pronounced for higher initial κ₀ values
- Good agreement between ODE and Fokker-Planck solutions, validating the reduced model

PHENOMENON DEMONSTRATED:
=======================
When cells start aligned near an energy maximum (μ₀ ≈ 0) with high initial concentration 
(κ₀ large), the reorientation process occurs in two stages:
1. First stage: The distribution broadens (κ decreases) as cells spread out
2. Second stage: The distribution refocuses (κ increases) as cells concentrate at 
   the energy minimum (μ → π/2)

This is a novel prediction of the model that can be tested experimentally.
"""
...
mu0 = 1e-2 * pi    # Initial mean angle at 
κ_values = [1 10.0 50.0] # 2.0 5.0 20.0]
...
odeSol = FP_solutions[3]
u = odeSol.u
plot(θ_grid, u[1:2:7,1], xlabel=L"\theta", ylabel=L"p(\theta,t)", 
     title="Fokker-Planck Solution at t = $(odeSol.t[end])", 
     label="", legend=:topright, grid=true, lw=2, color=:blue)
...
begin
    ...
    savefig("mu_comparison_second_spreading.png")
    display(p)
    ...
    savefig("kappa_comparison_second_spreading.png")
end
